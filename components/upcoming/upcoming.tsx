import {
  getUpcoming,
  getUpcomingQuotes,
} from "@/lib/services/upcoming.service";
...
export default async function Upcoming() {
  ...
  const quotes = await getUpcomingQuotes(upcoming_event.book.id);
      ...
      <div className="flex w-full h-4/5">
        ...
        <div className="w-3/5 flex justify-center items-center">
          <div className="flex flex-col justify-center items-center space-y-10 px-28">
            ...
            <p className="text-2xl font-serif">{upcoming_event.description}</p>
          </div>
        </div>
      </div>
      <div className="flex w-full h-1/5">
        <TextCarousel quotes={quotes} interval={8000} />
      </div>
    </div>
  );
}
